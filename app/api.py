...
import pandas as pd
from app.schemas import PredictPayload, Transaction
from config import config
from config.config import logger
from fraud_detection import main, predict

# Define application
app = FastAPI(
    title="Fraud Detection - ",
    description="Classify transactions for normal or genuine.",
    version="0.1",
)
...
def _predict(request: Request, payload: PredictPayload) -> Dict:
    """Predict tags for a list of transactions."""
    transaction = [trans for trans in payload.transactions]
    data = pd.DataFrame(data=data)
    predictions = predict.predict(data=transaction, artifacts=artifacts)
    response = {
        "message": HTTPStatus.OK.phrase,
        "status-code": HTTPStatus.OK,
